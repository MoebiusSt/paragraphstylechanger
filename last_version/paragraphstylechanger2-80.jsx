...
var ScriptVersion = '2.80';

var __readMe =  
'''
paragraphStyleChanger.jsx | v.2.80
-------------------------------------------------
by Stephan Möbius, last edited: August 2025.
...
v1.0 - May 2013 - A beefed up version of Thomas Silkjaer's 2009 script (Thanks Dude!), changes made to run in CS6,  to include styles in style-groups (not nesting), to save last settings, to give a report of num changed, and to add an -ANY-Style option for the follow up paragraphs.
...
''';
    var _logging = 1; //  0 = no logging  1 = to log-file  2 = to console
    ...
    var QL = function(sel /*QL_item*/,a /*array of QL_item*/) {
        ...
        updateSelectionInfo();
         ...
         if (!_isActive) {
            ...
            if (!_inDDactionFlag) {
                ...
                if(stylesChanged(_cachedSignatures) && !_docChanged) {
                    log('onActivate: styles did change - calling matchRenamed');
    ...
    function getStyleSignature(style) {
        try {
            return style.id + "|" + getNestingPath(style);
        } catch(e) {
    ...
    function getSafeStyles() {
        ...
        for(var i=0; i<app.activeDocument.allParagraphStyles.length; i++){
            try {
                styles.push(app.activeDocument.allParagraphStyles[i]);
            } catch(e) {/*Style überspringen*/}
        }
        return styles;
    }
    // ===========================================
    ...
    function stylesChanged(_cachedSignatures) 
    {
        ...
        try {
            ...
            if(signatures != _cachedSignatures) {
                ...
                log('stylesChanged: detected change - new cache: '+ _cachedSignatures)
                return true;
            }
            return false;
        }
