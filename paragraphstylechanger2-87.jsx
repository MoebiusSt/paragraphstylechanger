...
var ScriptVersion = '2.87';

var __readMe =  
'''
paragraphStyleChanger.jsx | v.2.87
-------------------------------------------------
by Stephan Möbius, last edited: 03. April 2026.
...
_____________
CHANGELOG
v2.87 - May 2026 - Fixed: on first start in default palette width, the dropdown popup list was rendered too wide because items were initially added with full style names (filling ScriptUI's internal popup-width cache) and only afterwards shortened via .text. New resyncShortened() prototype now adds items already shortened on insert, while keeping full names in properties.data for findInData() lookup. shortenDDTexts() is no longer called at the end of updateDDs().
v2.86 - April 2026 - Fixed: styleName shortening now adapt dynamically to available panel width and dropdowns re-populute after resize in order to adapt force an update to width.
v2.84 - December 2025 - Fixed: old version settings import not correctly updating status after import since version 2.82.
...
''';
    ...
    ListBox.prototype.resync = DropDownList.prototype.resync = function(/*obj[]&*/dp) {  
     ...
     // =====================================
    // Resync with display texts already shortened to the given character limit.
    // Full names are preserved in properties.data for lookup via findInData().
    // This prevents ScriptUI from caching a too-wide popup width based on the full names —
    // an artifact ScriptUI never shrinks afterwards (only widget recreation can shrink it).
    ListBox.prototype.resyncShortened = DropDownList.prototype.resyncShortened = function(/*obj[]&*/dp, /*int*/maxChars) {
        this.removeAll();
        this.properties || (this.properties = {});
        this.properties.data = dp || [];
        var n = (dp && dp.length) || 0;
        for (var i = 0; i < n; i++) {
            var s = '' + dp[i];
            this.add("item", s.shorten(maxChars));
        }
        return this;
    }
     // =====================================
    ...
    // =====================================
    // Find item by text first, then fallback to searching the data provider by full name.
    // Needed because dropdown item texts may be dynamically shortened for display.
    ListBox.prototype.findInData = DropDownList.prototype.findInData = function(str) {
        var result = this.find(str);
        if (result) return result;
        var dp = this.getData ? this.getData() : null;
        if (!dp) return null;
        var s = '' + str;
        for (var i = 0; i < dp.length; i++) {
            if (('' + dp[i]) === s || dp[i].name === s) {
                return this.items[i];
            }
        }
        return null;
    ...
    Group.prototype.createDD = function( /*str*/ ddName, /*str*/ ddTitle, /*str*/ ddDefault)
    {
        ...
        with(g = this["grp"+ddName] = this.add( "group", undefined, undefined )){
            ...
            with (dd = g[ddName] = add("dropdownlist")) {
                ...
                dd.resyncShortened = ListBox.prototype.resyncShortened;
                dd.getData = ListBox.prototype.getData;
                dd.findInData = ListBox.prototype.findInData;
                // Load initial data
    ...
    var _lastPaletteWidth = _wW; // tracks last known palette width for resize detection
    ...
    {  
        w.layout.resize ();
        // Only rebuild dropdown widgets when palette width actually changed
        try {
            var curW = w.size ? w.size[0] : _lastPaletteWidth;
            if (curW !== _lastPaletteWidth) {
                _lastPaletteWidth = curW;
                rebuildDDWidgets();
            }
        } catch(e) {}
    }
    ...
    var but_anyClickHandler = function(ev)
    {
        ...
        if (!dd.selection.index || dd.selection.index != 0) {
            _stopCalls = true; 
            dd.selection = dd.findInData((dd.name ==="ffp"||dd.name ==="fsp")?nameOfAnyStyle:nameOfDontChange);
            ql.selection = 0;
            _stopCalls = false;
            storeDDs();
        } else {
    ...
    var getStylesClickHandler = function(ev)
    {
        ...
        if(_o.ps.length > 0) {
            _stopCalls = true; 
            if (dd === "fsp") {
                ff.selection = ff.findInData(getNestingPath((_o.cp && _o.cp[0].appliedParagraphStyle) || _o.ps[0].appliedParagraphStyle));
                fs.selection = (_o.cp || _o.ps.length === 1) ? fs.findInData(nameOfAnyStyle) : fs.findInData(getNestingPath(_o.ps[1].appliedParagraphStyle));
            }
            if (dd === "csp") {
                cf.selection = cf.findInData(getNestingPath((_o.cp && _o.cp[0].appliedParagraphStyle) || _o.ps[0].appliedParagraphStyle));
                cs.selection = (_o.cp ||_o.ps.length === 1) ? cs.findInData(nameOfDontChange) : cs.findInData(getNestingPath(_o.ps[1].appliedParagraphStyle));
            }
...
w.p.s.getOldLbl.c.addEventListener("click", function() {
    ...
    if(docDialog.show() === 1) {
        ...
        if (previousLabels.length > 0) {
            // EXIT chain mode before importing (if active)
            if (_chainModeActive) {
                log('getOldLbl: exiting chain mode before import');
                exitChainMode();
            }
            // Reset chain selection to "no chain"
            ql.selChain = 0;
            if (w.p.m.grpChain && w.p.m.grpChain.selChain) {
                w.p.m.grpChain.selChain.selection = 0;
            }
            ...
            // Update dropdowns with imported settings
            updateDDs();
            storeDDs();
        } else {
            w.grp1.txt_info.text = localize({
    ...
    function updateQL(sourceDoc) {
        ...
        if (app.documents.length !== 0 ) {  
           ...
           // Backward compatibility: ensure chain mode state properties exist
           if (typeof qlDP.chainModeActive !== 'boolean') qlDP.chainModeActive = false;
           if (qlDP.cachedDDState === undefined) qlDP.cachedDDState = null;
        }
        ...
        if (w && w.p && w.p.m && w.p.m.grpChain) {
           ...
           // Only restore if we have valid cached state
           if (qlDP.chainModeActive && qlDP.selChain > 0 && !_chainModeActive && qlDP.cachedDDState) {
        ...
        try { 
            ...
            for (var DD in set) {
               ...
               // establishing DD selections, adding missing items if needed 
                ...
                //log(dp[DD].toSource());
                _stopCalls = true;
                // Use resyncShortened so items are added with already-truncated display texts.
                // This avoids ScriptUI caching a too-wide popup width based on the full names.
                // Full names remain in properties.data — selection is then resolved via findInData().
                var ddRef = w.p.m["grp"+DD][DD];
                var maxC = calcMaxCharsForDD(ddRef);
                ddRef.resyncShortened(dp[DD], maxC);
                ddRef.selection = ddRef.findInData(s) || null;
                _stopCalls = false;
                 log('-->' + DD + ' : ' + s);
            }
            // Note: shortenDDTexts() is no longer needed here — items are already shortened on add.
        }
        catch(e) {
    ...
    function getNestingPath (s) 
    {
        ...
        // NOTE: Returns full unshortened name. Display truncation is handled dynamically by shortenDDTexts/rebuildDDWidgets.
    ...
    // =====================================
    // Calculate the maximum number of characters that fit in a dropdown at its current width.
    // Uses an average character width heuristic for the ScriptUI dropdown font.
    function calcMaxCharsForDD (dd) {
        var ddWidth;
        try {
            ddWidth = dd.size ? dd.size[0] : dd.bounds[2] - dd.bounds[0];
        } catch(e) {
            ddWidth = _wW - 50; // fallback to default
        }
        // Subtract padding/chrome of the dropdown widget (approx 30px for arrow + internal padding)
        var usable = ddWidth - 30;
        if (usable < 40) usable = 40;
        // Average character width in ScriptUI's default proportional font is roughly 5.2px
        var maxChars = Math.floor(usable / 6.2);
        if (maxChars < 10) maxChars = 10;
        return maxChars;
    }
    // =====================================
    // Lightweight: only update display texts on existing items. Fast, no side effects.
    // Used after updateDDs() to shorten names for the current dropdown width.
    function shortenDDTexts () {
        var ddNames = ["ffp", "fsp", "cfp", "csp"];
        for (var d = 0; d < ddNames.length; d++) {
            var ddName = ddNames[d];
            var dd = w.p.m["grp" + ddName][ddName];
            var maxC = calcMaxCharsForDD(dd);
            var dp = dd.getData ? dd.getData() : null;
            if (!dp) continue;
            for (var i = 0; i < dd.items.length && i < dp.length; i++) {
                dd.items[i].text = ('' + dp[i]).shorten(maxC);
            }
        }
    }
    // =====================================
    // Heavy: RECREATE each dropdown widget entirely. Only called on actual palette resize.
    // ScriptUI caches popup list width internally and never shrinks it — even after removeAll().
    // Destroying and recreating the widget is the only reliable way to force recalculation.
    function rebuildDDWidgets () {
        var prevStop = _stopCalls;
        _stopCalls = true;
        var ddNames = ["ffp", "fsp", "cfp", "csp"];
        for (var d = 0; d < ddNames.length; d++) {
            var ddName = ddNames[d];
            var grp = w.p.m["grp" + ddName];
            var oldDD = grp[ddName];
            var maxC = calcMaxCharsForDD(oldDD);
            var dp = oldDD.getData ? oldDD.getData() : null;
            if (!dp) continue;
            // Save state from old widget
            var selIdx = oldDD.selection ? oldDD.selection.index : null;
            var handlerChange = oldDD.onChange || null;
            var handlerActivate = oldDD.onActivate || null;
            // Remove old dropdown widget from its parent group
            grp.remove(oldDD);
            // Create fresh dropdown widget (resets internal popup width cache)
            var newDD = grp.add("dropdownlist");
            newDD.name = ddName;
            newDD.alignment = ["fill", "center"];
            // Assign prototype methods (ScriptUI doesn't reliably inherit prototypes)
            newDD.load = DropDownList.prototype.load;
            newDD.resync = DropDownList.prototype.resync;
            newDD.resyncShortened = DropDownList.prototype.resyncShortened;
            newDD.getData = DropDownList.prototype.getData;
            newDD.findInData = DropDownList.prototype.findInData;
            // Preserve original full-name data provider for lookups
            newDD.properties = newDD.properties || {};
            newDD.properties.data = dp;
            // Add items with dynamically shortened display texts
            for (var i = 0; i < dp.length; i++) {
                newDD.add("item", ('' + dp[i]).shorten(maxC));
            }
            // Restore selection without triggering onChange
            if (selIdx !== null && selIdx < newDD.items.length) {
                newDD.selection = selIdx;
            }
            // Re-attach event handlers AFTER setting selection
            if (handlerChange) newDD.onChange = handlerChange;
            if (handlerActivate) newDD.onActivate = handlerActivate;
            // Update all references to point to the new widget
            grp[ddName] = newDD;
            if (ddName === "ffp") ff = newDD;
            else if (ddName === "fsp") fs = newDD;
            else if (ddName === "cfp") cf = newDD;
            else if (ddName === "csp") cs = newDD;
        }
        // Force complete relayout so the new widgets get correct sizes
        w.layout.layout(true);
        // Restore chain mode display if active (chain items are UI-only, not in data provider)
        if (_chainModeActive) {
            refreshChainModeItems();
        }
        _stopCalls = prevStop;
